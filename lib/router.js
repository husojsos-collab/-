...
async function botIsAdminInGroup(sock, chatId) {
  const meta = await sock.groupMetadata(chatId);
  const result = wa.isBotAdmin(meta, sock.user);
...
async function handleSingleMessage(sock, msg, imagesDir, persistDir) {
  ...
  if (isGroup && !fromMe && body) {
    messageLogger.logMessage(chatId, authorNumber, body);
  }
  ...
  if (body === '!قفل' || body === '!فتح') {
    ...
    if (body === '!قفل') {
      if (isCurrentlyLocked) {
        await wa.reply(sock, msg, banners.noChangeBanner('🔐⃝⚡ *الـمـجـمـوعـة مـقـفـولـة بـالـفـعـل*'));
        return;
      }
      ...
      if (!isCurrentlyLocked) {
        await wa.reply(sock, msg, banners.noChangeBanner('🔓⃝⚡ *الـمـجـمـوعـة مـفـتـوحـة بـالـفـعـل*'));
        return;
      }
  ...
  if (body.startsWith('!اقتباس')) {
    ...
    const picked = messageLogger.getRandomMessage(chatId, targetNumber);
  ...
  if (body.startsWith('!تحليل_شخصية')) {
    ...
    const targetJid = targets[0];
    const targetNumber = await wa.resolveRealNumberForJid(sock, chatId, targetJid);
    const history = messageLogger.getMessages(chatId, targetNumber);
  ...
  if (body === '!اوامر') {
    ...
    const header = `╔══════════════════════════════════════╗
⚡ 𝐓𝐎𝐆𝐈 𝐁𝐎𝐓 ⚡
𖤍 القوة • السرعة • الحماية 𖤍
╚══════════════════════════════════════╝

🤖 الاســم      : 𝐓𝐎𝐆𝐈 𝐁𝐎𝐓
🔥 الإصــدار    : v1.0
⚡ الــحــالــة : يــعــمــل
👨‍💻 الــمــطــور : 𝗛𝗢𝗦𝗦𝗔𝗠 𖤍
🛡️ الــنــظــام : Security AI
...
🔵 !مــراد [سؤال]
↜ اسأل مراد (تقدر تزيد "بأسلوب ...")

🔵 !ســعاد [سؤال]
↜ اسأل سعاد (تقدر تزيد "بأسلوب ...")
...
━━━━━━━━━━━━━━━━━━━━━━━━━━
🕌 ❰ أوقــات الــصــلاة (بالخاص بس) ❱
━━━━━━━━━━━━━━━━━━━━━━━━━━

🟢 !صلاة
↜ أوقات اليوم حسب دولتك

🟢 !تفعيل_تنبيه_الصلاة
↜ تنبيه تلقائي وقت كل أذان
