...
const staticFfmpegPath = require('ffmpeg-static'); // مسار ffmpeg الثابت، ما يعتمد على PATH النظام إطلاقًا

// حماية: أحيانًا حزمة ffmpeg-static تفشل تجيب نسخة ffmpeg وقت التثبيت على بعض
// السيرفرات (بترجع null بدل مسار حقيقي). بهالحالة نرجع نجرب "ffmpeg" من نظام
// التشغيل مباشرة (لو متثبت بالسيرفر كـ apt package مثلاً)، بدل ما نطيح فورًا.
const ffmpegPath = (staticFfmpegPath && fs.existsSync(staticFfmpegPath)) ? staticFfmpegPath : 'ffmpeg';
if (ffmpegPath === 'ffmpeg') {
  console.error(
    '⚠️ تحذير: حزمة ffmpeg-static ما جابت نسخة ffmpeg صحيحة بهالسيرفر (رجعت مسار فاضي/غير موجود).\n' +
    '   البوت رح يحاول يستخدم "ffmpeg" من نظام التشغيل مباشرة كبديل.\n' +
    '   لو ظهر خطأ "ENOENT" لاحقًا بأوامر الجيف/الفيديو، هذا معناه ffmpeg مو مثبت بالسيرفر إطلاقًا،\n' +
    '   ولازم يتثبت كبرنامج نظام (system package)، مو بس كحزمة npm.'
  );
}
