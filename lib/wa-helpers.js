...
async function resolveRealAuthorNumber(sock, msg) {
  ...
  // مش @lid أصلاً (خاص أو جروب برقم عادي) - الاستخراج المباشر كافي وصحيح
  if (!authorId.includes('@lid')) return jidToNumber(authorId);

  const chatId = getChatId(msg);
  if (!isGroupJid(chatId)) return jidToNumber(authorId);

  try {
    const meta = await sock.groupMetadata(chatId);
    const p = meta.participants.find((pp) => pp.id === authorId || pp.lid === authorId);
    if (p) {
      const real = bestDisplayNumber(p);
      if (real) return real;
    }
  } catch (err) {
    console.error('خطأ بجلب الرقم الحقيقي لصاحب الرسالة من الجروب:', err.message);
  }

  // fallback: ما قدرناش نحل الرقم الحقيقي - نرجع نفس السلوك القديم
  return jidToNumber(authorId);
}
