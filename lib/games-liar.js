...
function normalizeLiarAnswer(text) {
  ...
  if (['1', '١', 'واحد', 'الأولى', 'الاولى'].includes(clean)) return 1;
  if (['2', '٢', 'اتنين', 'الثانية', 'التانية'].includes(clean)) return 2;
  if (['3', '٣', 'تلاتة', 'ثلاثة', 'الثالثة', 'التالتة'].includes(clean)) return 3;
  return null;
}
