  ...
  if (body.startsWith('!بن')) {
    ...
    try {
      ...
      await sock.sendMessage(chatId, {
        ...
        caption: `🖼️ *${first.title}*\n🔗 ${first.pinUrl}\n\n(1/${results.length}) — اكتب !تالي عشان تشوف الجاية`
      });
    } catch (pinErr) {
  ...
  if (body.startsWith('!تك')) {
    ...
    try {
      ...
      await sock.sendMessage(chatId, {
        ...
        caption: `🎵 *${first.title}*\n👤 @${first.author}\n🔗 ${first.videoUrl}\n\n(1/${results.length}) — اكتب !تالي عشان تشوف الجاي`
      });
    } catch (tikErr) {
    ...
    try {
      if (advanced.type === 'pinterest') {
        await sock.sendMessage(chatId, {
          ...
          caption: `🖼️ *${item.title}*\n🔗 ${item.pinUrl}\n\n(${advanced.index + 1}/${advanced.results.length}) — اكتب !تالي للمتابعة`
        });
      } else {
        await sock.sendMessage(chatId, {
...
🟢 !صلاة
🟢 !تفعيل_تنبيه_الصلاة    !ايقاف_تنبيه_الصلاة`;
...
🟣 !اصعــد @          !انــزل @
🟣 !تشغيل            !توقف
🟣 !ازالــة_تحــذيــر @  !تغيير_صورة
🟣 !منشن [رسالة]`;

    const footer = `

━━━━━━━━━━━━━━━━━━━━━━━━━━
🚫 ❰ نــظــام الــتــحــذ يــرات ❱
━━━━━━━━━━━━━━━━━━━━━━━━━━
