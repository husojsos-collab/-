...
const COMMAND_GATES = [
  ...
  { key: 'عام', test: (b) => b === '!عام' },
  { key: 'العاب', test: (b) => b === '!العاب' },
  { key: 'ادارة', test: (b) => b === '!ادارة' },
...
function adminOnlyBanner() {
  return `╭━━━ 🌸 『 ✨ 𝑨𝑳𝒀𝑨-𝑪𝑯𝑨𝑵 ✨ 』 🌸 ━━━╮
┃  ⚠️ *تــنــبــيــه لــطــيــف مــن ألــيــا*
┣━━━━━━━━━━━━━━━━━━━━━━━━━━⫸
┃  🌸 هـاد الأمـر لـلـأدمـن بـس يا بـطـل! ✨
┃  💖 خـلـي الإدارة لـأهـلـهـا واسـتـمـتـع مـعـنـا!
╰━━━━━━━━━━━━━━━━━━━━━━━━━━╯

˼✨˹ ┃ *ALYA-CHAN تضمن النظام بألطف طريقة* 🎀`;
}
...
async function handleSingleMessage(sock, msg, imagesDir, persistDir) {
  ...
  if (body.startsWith('!صمم')) {
    ...
    if (!designPrompt) {
      await wa.reply(sock, msg, 'اكتب وصف الصورة بعد الأمر، مثال: !صمم قطة تلبس نظارة شمس 🎨');
      return;
    }
    try {
      await wa.reply(sock, msg, '🎨 عم أصمملك الصورة، ثواني...');
      ...
      await wa.reply(sock, msg, 'صار خطأ بالتصميم، جرب مرة ثانية 🥲');
    }
    return;
  }
  ...
  if (body.startsWith('!عدل')) {
    ...
    if (!quotedMessage || !quotedMessage.imageMessage) {
      await wa.reply(sock, msg, 'لازم ترد (Reply) على صورة موجودة بالجروب، وتكتب بعد !عدل شو التعديل يلي بدك ياه 🖼️\nمثال: !عدل خلي الخلفية شاطئ');
      return;
    }
    if (!editInstruction) {
      await wa.reply(sock, msg, 'اكتب وصف التعديل بعد الأمر، مثال: !عدل خلي الخلفية شاطئ 🎨');
      return;
    }
    try {
      await wa.reply(sock, msg, '🖌️ عم أعدّل الصورة، ثواني...');
      const media = await wa.downloadQuotedMedia(quotedMessage);
      if (!media) {
        await wa.reply(sock, msg, 'ما قدرت أحمّل الصورة، جرب مرة ثانية 🥲');
        return;
      }
      ...
      await wa.reply(sock, msg, 'صار خطأ بالتعديل، جرب مرة ثانية 🥲');
    }
    return;
  }
  ...
  if (body.startsWith('!اصعد')) {
    ...
    try {
      ...
      const banner = [
        ...
        '',
        `👤 *المشرف المميز :* ${promotedLine}`,
        `⏰ *التوقيت واليوم :* ${greetings.formatTripoliTime()}`,
        '✨ *الـحـالـة :* تم الانضمام لطاقم الإدارة 🌸',
        '',
        '🌸 ━━━━ »✥«🎀»✥« ━━━━ 🌸',
        '˼✨˹ ┃ألف مبروك الترقية يا بطل… 🎀',
        '˼💖˹ ┃منور الإدارة وواثقين فيك تزبط الأجواء💖',
        '˼🌸˹ ┃ALYA-CHAN تتمنى لك كل التوفيق🌸',
        '🌸 ━━━━ »✥«🎀»✥« ━━━━ 🌸'
      ].join('\n');
  ...
  if (body.startsWith('!انزل')) {
    ...
    try {
      ...
      const banner = [
        ...
        '',
        `👤 * العضو العزيز :* ${demotedLine}`,
        `⏰ *التوقيت واليوم :* ${greetings.formatTripoliTime()}`,
        '⚡ *الـحـالـة :* تم الإعفاء والعودة لرتبة عضو 🌸',
        '',
        '🌸 ━━━━ »✥«🎀»✥« ━━━━ 🌸',
        '˼✨˹ ┃شكراً كبيـرة على كل جهودك العظيمة… 🎀',
        '˼💖˹ ┃ترجع تنورنا كعضو مميز بعائلتنا💖',
        '˼🌸˹ ┃ALYA-CHAN تقدر كل أوقاتك معنا🌸',
        '🌸 ━━━━ »✥«🎀»✥« ━━━━ 🌸'
      ].join('\n');
...
قسّمنا الأوامر لـ 3 أقسام عشان تكون أسهل وأوضح 💕

🎀 !عام      → الأوامر العامة
🎮 !العاب    → أوامر التسلية والألعاب
👑 !ادارة    → أوامر الإدارة (للأدمن)

اكتب أي أمر منهم وبيطلع لك القسم كامل ✨
...
🌸 ━━━━ »✥«🎀»✥« ━━━━ 🌸
˼✨˹ ┃استمتعوا باللعب والمرح مع ALYA-CHAN… 🎀
˼💖˹ ┃نتمنى لكم أجواء ممتعة ولحظات حلوة💖
🌸 ━━━━ »✥«🎀»✥« ━━━━ 🌸`;
  ...
  if (body === '!ادارة') {
    ...
    if (!(await botIsAdminInGroup(sock, chatId))) {
      const botNotAdminBanner = `╭━━━ 🌸 『 ✨ 𝑨𝑳𝒀𝑨-𝑪𝑯𝑨𝑵 ✨ 』 🌸 ━━━╮
┃  ⚠️ *تــنــبــيــه مــن ألــيــا*
┣━━━━━━━━━━━━━━━━━━━━━━━━━━⫸
┃  🌸 أنا مش أدمن بهاد الجروب حالياً!
┃  💖 خلوني أدمن الأول باش أقدر أفعّل أوامر الإدارة 🙏
╰━━━━━━━━━━━━━━━━━━━━━━━━━━╯

˼✨˹ ┃ *ALYA-CHAN بانتظار صلاحياتها* 🎀`;
...
🌸 ━━━━ »✥«🎀»✥« ━━━━ 🌸
˼✨˹ ┃ALYA-CHAN لضمان النظام والأمان… 🎀
˼💖˹ ┃شكراً لجهودكم في إدارة المجموعة💖
🌸 ━━━━ »✥«🎀»✥« ━━━━ 🌸`;
