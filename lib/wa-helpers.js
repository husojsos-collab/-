...
// ============ مطابقة أي JID (رقم عادي أو @lid) مع صاحبه الفعلي بلستة المشاركين ============
// المشكلة: واتساب صار يرجّع بعض الأشخاص بصيغة @lid (معرف خصوصية) بدل رقم الهاتف
// الحقيقي. فلو المنشن رجع صيغة، ولستة المشاركين مسجلة بصيغة ثانية، أي مقارنة أو
// استدعاء API مباشر (زي groupParticipantsUpdate) بيفشل لأنه ما يلقى تطابق تام.
// هاي الدالة تدور على "المشارك الحقيقي" بغض النظر عن الصيغة، وترجع الـ id
// المضبوط زي ما هو مسجل بلستة المشاركين (عشان نستخدمه بأي استدعاء لواتساب).
function findParticipant(groupMeta, jid) {
  if (!jid) return null;
  const num = jidToNumber(jid);
  return (
    groupMeta.participants.find((pp) => {
      if (pp.id === jid) return true;
      const altFields = [pp.jid, pp.phoneNumber, pp.lid, pp.pn].filter(Boolean);
      if (altFields.includes(jid)) return true;
      if (num && jidToNumber(pp.id) === num) return true;
      if (num && altFields.some((f) => jidToNumber(f) === num)) return true;
      return false;
    }) || null
  );
}

// ترجع نفس الـ id المسجل فعلياً بلستة المشاركين (أدق صيغة لاستخدامها مع API واتساب)
function resolveParticipantId(groupMeta, jid) {
  const p = findParticipant(groupMeta, jid);
  if (!p) return jid; // لو ما لقينا تطابق، نرجع الأصلي بدل ما نرمي خطأ (fallback آمن)

  // مهم جداً: بعض الأعضاء (بسبب إعدادات خصوصية واتساب الجديدة) مسجلين بلستة
  // المشاركين بصيغة "@lid" كـ id افتراضي بدل رقم هاتفهم الحقيقي "@s.whatsapp.net".
  // عمليات الإدارة (ترقية/تنزيل/طرد groupParticipantsUpdate) واتساب يرفضها لو
  // استخدمنا صيغة @lid، ولازم نستخدم الصيغة الحقيقية برقم الهاتف. فلو الـ id
  // الأساسي كان @lid، نجرب نلقى صيغة بديلة حقيقية بنفس المشارك ونستخدمها بدله.
...
function isBotAdmin(groupMeta, botUser) {
  ...
  // السبب: واتساب توا بيرجع بعض المشاركين بالقروب بصيغة @lid (معرف مخفي للخصوصية)
  // بدل رقم الهاتف الحقيقي، فمقارنة رقم واحد بس كانت تفشل حتى لو البوت أدمن فعلاً.
  const identities = [];
  if (typeof botUser === 'string') {
    identities.push(botUser);
  } else if (botUser && typeof botUser === 'object') {
    if (botUser.id) identities.push(botUser.id);
    if (botUser.lid) identities.push(botUser.lid);
  }
  if (identities.length === 0) return false;

  const identityNumbers = identities.map(jidToNumber).filter(Boolean);

  const p = groupMeta.participants.find((pp) => {
    // 1) مطابقة مباشرة بالـ JID الكامل (أدق طريقة لو نفس الصيغة)
    if (identities.includes(pp.id)) return true;
    // 2) بعض نسخ Baileys بترجع حقول بديلة (jid/phoneNumber/lid/pn) جوا كل مشارك
    const altFields = [pp.jid, pp.phoneNumber, pp.lid, pp.pn].filter(Boolean);
    if (altFields.some((f) => identities.includes(f))) return true;
    // 3) fallback: مقارنة بالرقم المستخرج (بتشتغل لو الاتنين نفس صيغة الرقم العادي)
    if (identityNumbers.includes(jidToNumber(pp.id))) return true;
    return false;
  });
