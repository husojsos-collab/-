...
async function handleSingleMessage(sock, msg, imagesDir, persistDir) {
  ...
  if (body.includes('سعاد')) {
    try {
      ...
      if (jidToNumber(authorId) === husaamNumber) {
        const loveReplies = [
  ...
  if (!fromMe && jidToNumber(authorId) !== media.HUSAAM_MENTION_NUMBER) {
    const mentioned = wa.getMentionedJids(msg);
    const mentionsHusaam = mentioned.some((jid) => jidToNumber(jid) === media.HUSAAM_MENTION_NUMBER);
    if (mentionsHusaam) {
  ...
  if (body === 'حقوق بوت') {
    try {
      ...
      await sock.sendMessage(chatId, { video: videoBuffer, caption: huqoqCaption });
    } catch (huqoqErr) {
  ...
  if (body.startsWith('!باند')) {
    ...
    const mentioned = wa.getMentionedJids(msg);
    if (mentioned.length === 0) { await wa.reply(sock, msg, 'لازم تعمل منشن للعضو اللي بدك تطرده 📌'); return; }
  ...
  if (body.startsWith('!اصعد')) {
    ...
    const mentioned = wa.getMentionedJids(msg);
    if (mentioned.length === 0) { await wa.reply(sock, msg, 'لازم تعمل منشن للعضو اللي بدك ترفعه أدمن 📌 مثال: !اصعد @فلان'); return; }
  ...
  if (body.startsWith('!انزل')) {
    ...
    const mentioned = wa.getMentionedJids(msg);
    if (mentioned.length === 0) { await wa.reply(sock, msg, 'لازم تعمل منشن للعضو اللي بدك تنزله من الأدمن 📌 مثال: !انزل @فلان'); return; }
  ...
  if (body.startsWith('!ازالة_تحذير')) {
    ...
    if (!(await senderIsAdmin(sock, chatId, authorId))) { await wa.reply(sock, msg, 'هاد الأمر للأدمن بس يا بطل 🚫'); return; }
    const mentioned = wa.getMentionedJids(msg);
    if (mentioned.length === 0) { await wa.reply(sock, msg, 'لازم تعمل منشن للشخص اللي بدك تشيل تحذيراته 📌 مثال: !ازالة_تحذير @فلان'); return; }
  ...
  if (body.startsWith('!مخالفة') || body.startsWith('!مخالفه')) {
    ...
    if (!(await senderIsAdmin(sock, chatId, authorId))) { await wa.reply(sock, msg, 'هاد الأمر للأدمن بس يا بطل 🚫'); return; }
    const mentioned = wa.getMentionedJids(msg);
    if (mentioned.length === 0) { await wa.reply(sock, msg, 'لازم تعمل منشن للعضو اللي بدك تحذّره 📌 مثال: !مخالفة @فلان سبب التحذير'); return; }
    const target = mentioned[0];
