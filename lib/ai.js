...
// المفتاح: message key id (msg.key.id) بـ Baileys، القيمة: { persona: 'murad'|'souad', isVoice: true|false }
...
async function askAI(userMessage, history = [], personaKey = DEFAULT_PERSONA_KEY, systemPromptOverride = null) {
  ...
  try {
    ...
    if (!replyContent) {
      console.error('خطأ: Groq رجع رد فاضي (بدون استثناء) - بنرجع رسالة مشغول بدلها.');
...
function loadHistoryFromDisk(persistDir) {
  try {
    ...
    // نرجّع ذاكرة "مين بعت كل رسالة" (مراد/سعاد) عشان الرد العادي (بدون أمر) يشتغل
