...
async function botIsAdminInGroup(sock, chatId) {
  const meta = await sock.groupMetadata(chatId);
  return wa.isBotAdmin(meta, sock.user);
}
...
async function handleSingleMessage(sock, msg, imagesDir, persistDir) {
  ...
  if (body.startsWith('!باند')) {
    ...
    if (!(await senderIsAdmin(sock, chatId, authorId))) { await wa.reply(sock, msg, 'هاد الأمر للأدمن بس يا بطل 🚫'); return; }
    const meta = await sock.groupMetadata(chatId);
    if (!wa.isBotAdmin(meta, sock.user)) { await wa.reply(sock, msg, 'لازم تخليني أدمن أول عشان أقدر أطرد حدا 🙏'); return; }
...
🔹 !صــمــراد [نص] ↜ صوت مراد (ولد)
🔹 !صــسعاد [نص] ↜ صوت سعاد (بنت)
...
⌬──══─┈•⤣⚡⤤•┈─══──⌬`;
  ...
  if (body.startsWith('!سعاد')) {
    ...
    const isLoved = ai.SOUAD_LOVED_NUMBERS.includes(jidToNumber(authorId));
    const replyText = await ai.askAI(prompt, history, 'souad', ai.getSouadSystemPrompt(isLoved));
  ...
  if (aiEnabled && wa.hasQuotedMessage(msg) && body.length > 1 && !body.startsWith('!')) {
    ...
    if (ai.isTrackedBotMessage(quotedInfo.stanzaId)) {
      ...
      const promptOverride = personaKey === 'souad'
        ? ai.getSouadSystemPrompt(ai.SOUAD_LOVED_NUMBERS.includes(jidToNumber(authorId)))
        : null;
  ...
  async function handleVoiceCommand(commandPrefix, forcedPersonaKey) {
    let textToSpeak = body.replace(commandPrefix, '').trim();
    let personaKeyForVoice = forcedPersonaKey || ai.DEFAULT_PERSONA_KEY;

    if (wa.hasQuotedMessage(msg) && textToSpeak.length === 0) {
      const quotedInfo = wa.getQuotedInfo(msg);
      // لو ما فيه فرض لشخصية معينة، وكانت الرسالة المقتبسة من مراد أو سعاد، ناخد صوتها هي
    ...
    if (textToSpeak.length === 0) {
      await wa.reply(sock, msg, `اكتب النص اللي بدك تحوله صوت، أو رد (Reply) على رسالة بـ ${commandPrefix} 🎙️`);
      return;
    }
